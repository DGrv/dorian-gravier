...
FOR /L %%p IN (0, 1, 2000) DO (
    ...
    set p2=!p2:~-5!
	echo [DEBUG]
	echo url loop: "%dirnameV%/segment-%%p.m4s"
	echo output loop: "!p2!.m4s"
	echo wget --no-check-certificate --user-agent Mozilla/4.0 "%dirnameV%/segment-%%p.m4s" -O "!p2!.m4s" -o log
	echo.
	wget --no-check-certificate --user-agent Mozilla/4.0 "%dirnameV%/segment-%%p.m4s" -O "!p2!.m4s" -o log
...
wget --no-check-certificate --user-agent Mozilla/4.0 "%aud%" -O "audio.mp3" -o log
