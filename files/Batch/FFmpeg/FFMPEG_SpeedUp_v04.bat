...
for /F "usebackq tokens=*" %%p in (%file%) do (
	...
	set RVfile=!RVdt:1/=!
	del tempfile
	ffmpeg -stats -loglevel error -y -i "%%p" -af "atempo=%speed%" -vf "setpts=PTS/%speed%" temp.mp4
	ffmpeg -stats -loglevel error -i temp.mp4 -video_track_timescale !RVfile! "%%~np_f%speed%%%~xp"
