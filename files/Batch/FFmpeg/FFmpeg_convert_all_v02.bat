...
for /F "usebackq tokens=*" %%i in (%file%) do (
	if %aud%==y (
		ffmpeg -i %%i -vcodec libx264 -vbr 3 -vf "scale=%reso%:-2" -preset fast -crf 23 -acodec mp3 %%~ni_low.mp4
	) else (
		ffmpeg -i %%i -vcodec libx264 -vbr 3 -vf "scale=%reso%:-2" -preset fast -crf 23 %%~ni_low.mp4
	)
)
