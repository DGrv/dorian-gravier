...
REM v08 is done for script mpv, in order to be able to give a txt file with the lines you want
REM v08 is done for script mpv, in order to be able to give a txt file with the lines you want
REM v08 is done for script mpv, in order to be able to give a txt file with the lines you want
REM v08 is done for script mpv, in order to be able to give a txt file with the lines you want
REM v08 is done for script mpv, in order to be able to give a txt file with the lines you want
...
if "%4"=="" (
	set /p inputfile="If you want to read text lines from file, give the path here, otherwise type enter: "
) else (
	set inputfile=%4
)

if "%5"=="" (
	set /p outputfile="Name of your output file: "
) else (
	set ouputfile=%5
)
...
if "%inputfile%"=="" (
	...
	for /f %%i in ('cat %inputfile% ^| wc -l') do set lines=%%i
)
...
if "%inputfile%"=="" (
	...
	FOR /l %%x IN (1, 1, %lines%) DO (
		sed -n "%%x{p;q}" %inputfile% >temp%%x.txt
		REM for /f %%i in ('sed -n "%%x{p;q}" %inputfile%') do set title%%x=%%i
		REM echo [95m title%%x = %%i [0m
	)
)
