...
FOR /l %%x IN (1, 1, %lines%) DO (
	...
	::echo high !high!
	if %%x==1 (
		ffmpeg -stats -loglevel error  -f lavfi -i color=c=black:s=2704x1520:d=5 -vf drawtext="fontfile=arial.ttf:fontsize=60:fontcolor=white:x=(w-text_w)/2:y=((h-text_h)/2)+!high!:text='| !title! |'" -video_track_timescale 30000 0000000%%x.mp4
	) else (
		ffmpeg -stats -loglevel error  -i 0000000!before!.mp4 -vf drawtext="fontfile=arial.ttf:fontsize=50:fontcolor=white:x=(w-text_w)/2:y=((h-text_h)/2)+!high!:text='!title!'" -video_track_timescale 30000 0000000%%x.mp4
