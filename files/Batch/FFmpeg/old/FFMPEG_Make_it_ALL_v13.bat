...
echo.
	...
	set /a n=0
	for /f %%i in ('ls ^| grep ".mp3" ^| wc -l') do set all=%%i	set /a n=0
...
echo.
	...
	set /a n=0
	for /f %%i in ('ls ^| grep ".mp3" ^| wc -l') do set all=%%i
			...
			if not "%%b"=="00000_title.mp4" (
				for /F "delims=" %%a in (Music_list_overlay_temp.txt) do (
					...
					if !videoTT! GTR !duraTT! (
						...
						echo ffmpeg -stats -loglevel error -i %%~nb_temp.mp4 -vf "drawtext=textfile=tempfilemusic: fontcolor=white: fontsize=20: box=1: boxcolor=Black@0.5:boxborderw=5: x=w*%xpos%-text_w:y=h*%ypos%:enable='between(t,!posmusic!,!posmusic2!)'" -vcodec libx264 -x264-params keyint=24:scenecut=0 -c:a copy -video_track_timescale %RV% %%b
						ffmpeg -stats -loglevel error -i %%~nb_temp.mp4 -vf "drawtext=textfile=tempfilemusic: fontcolor=white: fontsize=20: box=1: boxcolor=Black@0.5:boxborderw=5: x=w*%xpos%-text_w:y=h*%ypos%:enable='between(t,!posmusic!,!posmusic2!)'" -vcodec libx264 -x264-params keyint=24:scenecut=0 -c:a copy -video_track_timescale %RV% %%b
						del %%~nb_temp.mp4
						set /a duraTT+=dura
					)
				)
