...
echo.
	...
	REM echo ^| %title% ^| >> temptitle
	REM truncate -s -2 temptitle
	...
	if NOT EXIST 00000_title.mp4 (
		REM if %biketrip%==y (
			REM ffmpeg -stats -loglevel error -i "D:\Pictures\Youtube\Tatoo\Tatoo_v02.mp4" -vf drawtext="fontfile='C\:\\Windows\\Fonts\\calibri.ttf':fontsize=60:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2:textfile=temptitle" -video_track_timescale %RV% 000_temp.mp4
			REM ffmpeg -stats -loglevel error -i 000_temp.mp4 -f lavfi -i aevalsrc=0 -shortest -y 000_temp2.mp4
		REM ) 
		
		bash -c "echo -e '%title%' > temptitle"
		call C:\Users\doria\Downloads\GitHub\dorian.gravier.github.io\files\Batch\FFmpeg\Create_title_clip_v08___FFMPEG.bat 2 y 70 temptitle
		del temptitle
	)
