...
FOR /l %%x IN (1, 1, %lines%) DO (
	...
	if %%x==1 (
		if "!title%%x:~0,1!"=="*" (
			ffmpeg -stats -loglevel error -f lavfi -i color=c=black:s=2704x1520:d=%sec% -vf drawtext="fontfile=ARIALBD.TTF:fontsize=50:fontcolor=white:x=%xpos%:y=((h-text_h)/2)+!high!:text='!title%%x:~1,-1!'" -video_track_timescale 24000 0000000%%x.mp4
		) else (
		...
		if "!title%%x:~0,1!"=="*" (
			ffmpeg -stats -loglevel error -i 0000000!before!.mp4 -vf drawtext="fontfile=ARIALBD.TTF:fontsize=50:fontcolor=white:x=%xpos%:y=((h-text_h)/2)+!high!:text='!title%%x:~1,-1!'" -video_track_timescale 24000 0000000%%x.mp4
		) else (
