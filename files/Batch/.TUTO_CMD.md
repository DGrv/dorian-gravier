...
https://jonlabelle.com/snippets/language/dos
	...
	## Variables
		...
		### Assign output command to variable
			...
			Best solution ever found: https://stackoverflow.com/questions/14952295/set-output-of-a-command-as-a-variable-with-pipes
			
			```shell
			(echo foo & echo.) >2 & (set /p bar=)<2
			echo%bar%
			
			(sed -nr "s/.*<time>(.*)<\/time>/\1/p" "C:\Users\doria\Downloads\ori_gpx\004.gpx" | head -n1 & echo.) >2 & (set /p dayy=)<2
