...
REM :: get all links from the balise
REM copy %input% %input%BU
REM sed -i "s/<a href=\"/\r\n/g" %input%
REM sed -i "s/\" data\-nid\=.*$//g" %input%
REM sed -i "s/^/https\:\/\/www.thecrag.com/" %input%
REM :: delete 2 first lines
REM sed -i "1,2d" %input%
REM ls | grep "^sed.*" | xargs rm




REM cat %input% | sed "s/$/\/gpx/" | sed "s/^/wget --user-agent=Mozilla --content-disposition /" > temp.bat
REM uniq temp.bat > temp_uniq.bat
REM rm temp.bat
REM echo exit >> temp_uniq.bat
REM start /wait temp_uniq.bat
