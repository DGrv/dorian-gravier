...
for(i in 1:nrow(ll)){
  ...
  p <- ggplot(data[Name == ll$Name[i]], aes(x = dist, y = ele, fill = lcolor[i])) +
    geom_polygon()+
    geom_line(linewidth = 0.2)+
    scale_fill_manual(values = lcolor[i], guide = "none")+
    # scale_fill_discrete()+
  ...
  # Save as SVG
  setwd(rP("file:///C:/Users/doria/Downloads/gdrive/RR/2025/20250413__ZuerichMarathon/Course/svg/v04"))
  namesvg <- p0(ll$Name[i], ".svg")
  ggsave(namesvg, plot = p, device = "svg", width = 4000, height = 1000, units = "px")
  ...
  write.table(t, namesvg, row.names = F, col.names = F, quote = F)
