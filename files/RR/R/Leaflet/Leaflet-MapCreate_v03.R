...
loutput <- c(p0("OverviewMap__", EventName, ".html"), p0("OverviewMap__SmallCircle__", EventName, ".html"))


for(i in seq_along(loutput)) {
  
  outfilename <- loutput[i]
  
  if(i==1) {
    saveWidget(m, file=outfilename)
  } else {
    saveWidget(msmallcircle, file=outfilename)
  }
  ...
  if(i==1) {
    ...
    tadd <- readLines(rP("file:///C:/Users/doria/Downloads/GitHub/dorian.gravier.github.io/files/RR/Javascript/TrackingBox_CLEAN_v04.html"))
    tadd <- gsub("\\#idwidget", p0("#", idwidget), tadd)
