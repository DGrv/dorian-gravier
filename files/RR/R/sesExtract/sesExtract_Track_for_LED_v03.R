...
for(i in seq_along(ll$file) ) {
  ...
  if( exists("splits") ) {
    ...
    tempTP <- tp[TimingPoint %in% tempsplits$TimingPoint][grepl("start|Finish", TimingPoint, ignore.case = T)][!grepl("close_Finish", TimingPoint, ignore.case = T)]
  } else {
    tempTP <- tp[grepl("start|Finish", TimingPoint, ignore.case = T)][!grepl("close_Finish", TimingPoint, ignore.case = T)]
  }
