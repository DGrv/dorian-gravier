...
input.gpx <- rP("file:///C:/Users/doria/Downloads/gdrive/RR/2024/20240810_SwissAlps100/Course/Swiss%20Alps%20100%20-%20160KM.gpx")
output.name <- gsub(".gpx", ".svg", basename(input.gpx))


# GPX --------------------------------------------------------------------

data <- read.gpx(input.gpx, type="trk")
...
t <- data.table(raw=readLines("temp.svg"))
...
write.table(t$raw2, output.name, row.names = F, col.names = F, quote = F)
