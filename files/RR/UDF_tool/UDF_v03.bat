:: Convest json in cv
qsv json "User Defined Fields_Fcts..lvs" | qsv fmt --quote-always > UDF_ori.csv
::join but remove the one in the google sheet
qsv fmt --quote-always "UDF_template_v01 - List_UDF.csv" > UDF_gdrive.csv
...
qsv join --left Name UDF_ori.csv Name "UDF_template_v01 - List_UDF.csv" | qsv search -s Name[1] -v "^$" | qsv select "Name[1]-Expression[1]" > UDF_compare_gdrive.csv
...
C:\Users\doria\scoop\apps\git\current\usr\bin\rm.exe UDF_ori_filtered.json UDF_from-template.json UDF_ori.csv UDF_ori_filtered.csv temp.lvs UDF_gdrive.csv *.stats.csv.* *.stats.csv UDF_compare_gdrive.csv UDF_compare_event.csv 
