(function () {

    /**
     * Sends a status update message from the Page World to the Content Script World.
     * @param {string} text The text to display.
     */
    function reportStatus(text) {
        window.postMessage({
            type: "STATUS_UPDATE", // A unique message type
            message: text
        }, "*");
    }

    // At the top of the file, or just before deleteNextItem, add this function:
    function clickSectionHeader() {
        const listHeader = [...document.querySelectorAll(".SectionHeader")]
            .find(div => ["Lists", "Listen", "Listes / Éditions", "Liste", "Listas"].includes(div.innerText.trim()));

        if (listHeader) {
            listHeader.click();
        }
    }


    // 1️⃣ Find the SectionHeader div containing "Lists"
    const sectionHeader = [...document.querySelectorAll(".SectionHeader")]
        .find(div => ["Lists", "Listen", "Listes / Éditions", "Liste", "Listas"].includes(div.innerText.trim()));
    if (!sectionHeader) {
        console.warn("SectionHeader 'Lists' not found");
    } else {
        // 2️⃣ Get all .Item elements inside the same container
        const container = sectionHeader.nextElementSibling; // usually the list follows the header
        const items = [...container.querySelectorAll(".Item")];

        const totalItems = items.length;
        let currentIndex = 0;
        let finished = false;

        // Override window.confirm so deletion is auto-confirmed
        const oldConfirm = window.confirm;
        window.confirm = () => true;
        function deleteNextItem() {
            if (currentIndex >= totalItems) {
                if (!finished) {
                    finished = true;
                    reportStatus("✅ Delete all finished");
                    // Call the click function right after reporting completion
                    setTimeout(clickSectionHeader, 500); // Use a small delay
                }
                return;
            }
            reportStatus("⏳ Deleting list " + (currentIndex + 1) + "/" + items.length);

            const item = items[currentIndex];
            if (!item) {
                currentIndex++;
                setTimeout(deleteNextItem, 400);
                return;
            }

            // ✅ Open gear menu first
            const menuDiv = item.querySelector("div");
            if (menuDiv) menuDiv.click();

            // Then find the Delete row
            const deleteRow = [...document.querySelectorAll(".divOperations tr")]
                .find(tr => tr.innerText.includes("Delete"));

            if (deleteRow) {
                deleteRow.click();
            } else {
                console.warn("DELETE_ALL: Delete row not found for item", item.fullname || item.innerText);
            }

            currentIndex++;
            setTimeout(deleteNextItem, 600);
        }
    }


    deleteNextItem();

})();
